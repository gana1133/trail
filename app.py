...
app.config["SQLALCHEMY_DATABASE_URI"] = f"postgresql://postgres:{escaped_password}@localhost:5432/users"
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False
app.config["SQLALCHEMY_BINDS"] = {
    "new_db": f"postgresql://postgres:{escaped_password}@localhost:5432/faculty_db",  # PostgreSQL for faculty
    "perm_db": f"postgresql://postgres:{escaped_password}@localhost:5432/permissions_db"   # PostgreSQL for permissions
}
...
def process_request_post():
    ...
    # Create QR code data
            ...
            Start Time: {permission_request.start_time or 'N/A'}
            End Time: {permission_request.end_time or 'N/A'}
            Start Date: {permission_request.start_date or 'N/A'}
            End Date: {permission_request.end_date or 'N/A'}
            Permission ID: {permission_request.id}  <!-- Add this line -->
            """
...
def check_expired_permissions_and_notify():
    with app.app_context():
        try:
            ...
            print(f"Error in check_expired_permissions_and_notify: {str(e)}")
            traceback.print_exc()  # Print the full traceback
...
def check_in():
    try:
        ...
        db.session.rollback()
        print("Error during check-in:", str(e))  # Debug log
...
if __name__ == '__main__':
    ...
    scheduler.add_job(func=delete_expired_qr_codes, trigger="interval", hours=1)
      # Run every 2 minutes
...
# ✅ Full app.py final Render-ready snippet to add at end of your app.py

# Make sure this is at the bottom of app.py and not inside any route or function

if __name__ == "__main__":
    import os
    port = int(os.environ.get("PORT", 5000))  # Render assigns dynamic port
    app.run(host="0.0.0.0", port=port, debug=False)  # 0.0.0.0 makes Flask accessible publicly
